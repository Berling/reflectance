...
public:
	linked_property(const std::string& name, const getter_type<ValueType, OwnerType>& getter,
	...
	virtual accessor_type<ValueType> get(const BaseType& instance) const {
		...
		if(!_getter) {
			throw bad_accessor_call{"no accesor of type 'getter' specified in property " + abstract_property::name()};
		}
	...
	virtual void set(BaseType& instance, accessor_type<ValueType> value) {
		...
		if(!_setter) {
			throw bad_accessor_call{"no accessor of type 'setter' specified in property " + abstract_property::name()};
		}
...
template <typename ValueType, typename OwnerType, typename BaseType = OwnerType>
auto make_linked_property(const std::string& name, const getter_type<ValueType, OwnerType>& getter,
													const setter_type<ValueType, OwnerType>& setter) {
