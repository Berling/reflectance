...
#include <reflectance/assigner.hpp>
...
namespace reflectance {
	...
	private:
		...
		std::unordered_map<size_t, std::unique_ptr<abstract_assigner>> _assigners;
	...
	public:
		...
		void emplace(const std::string& name,
								 const getter<ValueType, OwnerType>& getter,
								 const setter<ValueType, OwnerType>& setter) {
			_property_names[name] = _properties.size();
			_properties.emplace_back(
			make_linked_property<ValueType, OwnerType, BaseType>(name, getter.value, setter.value));
			_assigners[typeid(ValueType).hash_code()]
			= std::make_unique<reflectance::assigner<ValueType, OwnerType, BaseType>>();
		}
		...
		auto& at(std::vector<std::unique_ptr<abstract_property>>::size_type index) {
			return *_properties.at(index);
		}

		auto& at(std::vector<std::unique_ptr<abstract_property>>::size_type index) const {
			return *_properties.at(index);
		}
		...
		auto& assigners() const noexcept {
			return _assigners;
		}

		auto& assigner(size_t hash_code) const {
			return *_assigners.at(hash_code);
		}
	};
}
