	...
	public:
		...
		any(const any& other) : _type{other._type} {
			if (other._stack_allocated) {
				std::memcpy(&_stack_allocated_contained_value, &other._stack_allocated_contained_value,
										3 * sizeof(size_t));
		...
		any(std::enable_if_t<!std::is_same<std::decay_t<ValueType>, any>::value, ValueType>&& value) {
			...
			if (sizeof(decayed_value_type) <= 3 * sizeof(size_t)
					&& std::is_trivially_copyable<decayed_value_type>::value) {
